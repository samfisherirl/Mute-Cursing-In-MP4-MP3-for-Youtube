...
CURSE_WORDS = str(Path(__file__).parent / "curse_words.csv")
...
def select_transcript():
    ...
    if messagebox.askyesno('Load Transcript', 'If this program crashed, it saves the transcript to ensure it doesn\'t require restarting.\n\nDo you want to load an existing transcript?'):
...
def transcriber(audio_file):
    ...
    model = stable_whisper.load_faster_whisper(
        size, device=device_type, device_index=0, compute_type='float16')
    if device_type == "cpu":
        print("\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@\nit is strongly recommended to setup pytorch \n(must be downloaded from pytorch site)\n and added to system path variables")
...
def apply_fade(audio_data, start_sample, end_sample, sample_rate, fade_percentage=0.1):
...
def censor(in_transcript, audio_file):
    """
...
if __name__ == '__main__':
    ...
    if not Path(uncensored_content).name.endswith('.mp4'):
